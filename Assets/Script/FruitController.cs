...
public class FruitController : MonoBehaviour
{
    ...
    IEnumerator TrySwap(FruitCell a, FruitCell b)
    {
        ...
        yield return StartCoroutine(BackFruit(a, b, tempA, tempB));
    }
    ...
    IEnumerator HandleFruitSpecial(FruitCell a, FruitCell b)
    {
        ...
        else if (!specialType.Contains(bType)&& specialType.Contains(aType))
        {
            ...
            yield return StartCoroutine(a.transform.GetChild(0)?.GetComponent<FruitSpecial>().ActiveEffect(a, b));
            *//*if (aType == FruitType.Rubik)
                yield return new WaitForSeconds(1f);
            else
                yield return new WaitForSeconds(0.1f);*//*
            yield return StartCoroutine(WaitToFallAndSpawn());
            //yield return StartCoroutine(FallAfterActive());

        }

        else if (specialType.Contains(bType) && !specialType.Contains(aType))
        {
            yield return StartCoroutine(b.transform.GetChild(0)?.GetComponent<FruitSpecial>().ActiveEffect(a, b));
            *//*if (bType == FruitType.Rubik)
                yield return new WaitForSeconds(1f);
            else
                yield return new WaitForSeconds(0.1f);*//*
            yield return StartCoroutine(WaitToFallAndSpawn());
    ...
    public Dictionary<IEnumerator, Coroutine> fruitCoroutines = new Dictionary<IEnumerator, Coroutine>();

    private IEnumerator FallAfterActive()
    {
        while (fruitCoroutines.Count > 0)
        {
            yield return null;
            Debug.Log("So luong: " + fruitCoroutines.Count);
        }

        Debug.Log("So luong s: " + fruitCoroutines.Count);
        yield return StartCoroutine(WaitToFallAndSpawn());
    }
    ...
    public void StartTrackedCoroutine(IEnumerator routine)
    {
        ...
        Coroutine coroutine = StartCoroutine(WrappedCoroutine(routine));
        fruitCoroutines[routine] = coroutine;
        Debug.Log("so luong: "+ fruitCoroutines.Count);
    }
    ...
    {
        yield return StartCoroutine(routine); // wait for the actual routine to finish

        // Remove from dictionary once it's done
        if (fruitCoroutines.ContainsKey(routine))
            fruitCoroutines.Remove(routine);
    }
