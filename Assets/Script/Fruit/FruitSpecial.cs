...
public abstract class FruitSpecial : MonoBehaviour
{
    ...
    [SerializeField] protected GameObject MissileWithBombPrefab;
    [SerializeField] protected GameObject MissileWithMissilePrefab;
    ...
    {
        if (index == 0)
            MissileWithMissileCase(b, a);
        else if (index == 1)
            MissileWithBombCase(b,a);
        else if (index == 2)
    ...
    protected void MissileWithMissileCase(FruitCell b, FruitCell a)
    {
        List<FruitCell> cells = new List<FruitCell>();
        foreach (FruitCell f in board.fruitCells)
        {
        ...
        GameObject he = Instantiate(MissileWithMissilePrefab, this.gameObject.transform.position, Quaternion.identity);
        ...
        CoroutineRunner.Instance.RunCoroutine(he.GetComponent<MissileWithMissileEffect>().Active(cells, this.transform, b));

        b.GetComponent<FruitCell>().GetFruit()?.GetComponent<Fruit>()?.Destroyit(0.1f);
        a.GetComponent<FruitCell>().GetFruit()?.GetComponent<Fruit>()?.Destroyit(0.1f);
        
    }
    
    protected void MissileWithBombCase(FruitCell b,FruitCell a)
    {
        List<FruitCell> cells = new List<FruitCell>();
        foreach (FruitCell f in board.fruitCells)
        {
        ...
        GameObject he = Instantiate(MissileWithBombPrefab, this.gameObject.transform.position, Quaternion.identity);
        he.transform.SetParent(GameObject.Find("HandleEffectPos").transform);

        //StartCoroutine(he.gameObject.GetComponent<MissileHorEffect>().Active(cells, this.transform));
        CoroutineRunner.Instance.RunCoroutine(he.GetComponent<MissileWithBombEffect>().Active(cells, this.transform, b));
